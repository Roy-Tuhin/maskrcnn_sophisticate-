...
function run_batch_train_evaluate(){
  ...
  source ${CURRENT_DIR}/batch_train_evaluate_cfg.sh
  source ${CURRENT_DIR}/batch_train_evaluate_workload.sh
  ...
  for annon_dbname in "${annon_dbnames[@]}"; do
    ...
    # local expid=${experiment_ids[${index}]}
    local expid=$(grep "CFG(Annotation Database) res:" ${prog_log} | rev | cut -d' ' -f1 | rev | tail -n 1)

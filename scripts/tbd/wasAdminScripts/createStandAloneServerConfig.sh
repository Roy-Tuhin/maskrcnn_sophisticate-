...
sh $wasRoot/profiles/$customProfileName/bin/wsadmin.sh -lang jython -f $scriptHome/wasAdminScripts/getports.py
sh $scriptHome/wasAdminScripts/writeports.sh
sh $wasRoot/profiles/$customProfileName/bin/wsadmin.sh -lang jython -f $scriptHome/wasAdminScripts/standaloneServerCreation.py $hostname -conntype SOAP -user $adminUserName -password $adminPassword
fi

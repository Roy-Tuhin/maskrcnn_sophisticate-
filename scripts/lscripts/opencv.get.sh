...
if [ -z "$OpenCV_VER_CHECKOUT" ]; then
  OpenCV_VER_CHECKOUT="3.3.0"
  OpenCV_VER_CHECKOUT="3.4.1"
  echo "Unable to get OpenCV_VER_CHECKOUT version, falling back to default version#: $OpenCV_VER_CHECKOUT"
fi

DIR='opencv'
PROG_DIR="$BASEPATH/$DIR"

URL="https://github.com/opencv/$DIR.git"
...
cd $PROG_DIR
git checkout $OpenCV_VER_CHECKOUT
cd $BASEPATH
...
URL2="https://github.com/opencv/opencv_contrib.git"

...
function cloudcompare_install() {
  ...
  if [ -z "${CloudCompare_REL}" ]; then
    local CloudCompare_REL="2.0.1"
    echo "Unable to get CloudCompare_REL version, falling back to default version#: ${CloudCompare_REL}"
  fi

  local DIR="CloudCompare"
  local PROG_DIR="${BASEPATH}/${DIR}"

  local URL="https://github.com/CloudCompare/${DIR}.git"
  # local URL="https://github.com/cloudcompare/trunk.git"

...
if [ -z "$PDAL_RELEASE" ]; then
  PDAL_VER="1.7"
  PDAL_BUILD="1"
  PDAL_RELEASE="$PDAL_VER.$PDAL_BUILD"
  echo "Unable to get PDAL_RELEASE version, falling back to default version#: $PDAL_RELEASE"
fi

PROG='PDAL'
DIR="$PROG-$PDAL_RELEASE-src"
PROG_DIR="$BASEPATH/$PROG-$PDAL_RELEASE-src"
FILE="$DIR.tar.gz"
...
cmake -D BUILD_PLUGIN_HEXBIN=ON \
      -D WITH_LAZPERF=ON \
      -D WITH_LZMA=ON ..
      # -D BUILD_PLUGIN_PYTHON=ON ..  ## error on Ubuntu 18.04 LTS 

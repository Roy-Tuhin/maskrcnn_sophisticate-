...
def create_data_split(img_lbl_arr, labels, splited_indices_per_label, already_processed_indices, prcntg):
  ...
  * all labels must split proportionally in the given criteria
  ...
  if total_images > 0 and len(labels) > 0:
    ...
    log.info("len(already_processed_indices), already_processed_indices: {}".format(len(already_processed_indices), already_processed_indices))
    ...
    if len(K) > 0:
      ...
      tt = T - tn - vl
      log.info("train, val, test splits: {},{},{}".format(tn, vl, tt))

      assert T == tn+vl+tt
      log.info("tn, tn+vl, tt: {},{},{}".format(tn, tn+vl, tt))

      ## get the indices per dataset splits
      split_pts = [[0,tn-1], [tn,tn+vl-1], [tn+vl,T-1]]
      log.info("split_pts: {}".format(split_pts))

      log.info("K[0:tn], K[tn:tn+vl], K[tn+vl:T]: {},{},{}".format(K[0:tn], K[tn:tn+vl], K[tn+vl:T]))
      
      if label not in splited_indices_per_label:
        splited_indices_per_label[label] = {}
      else:
        log.info("label already exists!!! raise Error")
      ...
      log.info("splited_indices_per_label: {}".format(splited_indices_per_label))
      if len(labels) > 0:
        log.info("index: {}, labels: {}".format(index, labels))
        labels.pop(index)
...
def do_data_split(cfg, images, labels, img_lbl_arr):
  ...
  log.info("splited_indices_with_duplication: {}".format(splited_indices_with_duplication))

...
def gen_train_sample(to_dir, b_gt_image_dir, i_gt_image_dir, image_dir):
    """
    ...
    with open('{:s}/training/train.txt'.format(to_dir), 'w') as file:
...
def process_tusimple_dataset(src_dir):
    ...
    to_path = '/aimldl-dat/data-gaze/AIML_Aids/'
    to_dir = os.path.join(to_path,'lnd-'+ timestamp)
    log.info("to_dir : {}".format(to_dir))
    os.makedirs(to_dir, exist_ok=True)

    traing_folder_path = ops.join(to_dir, 'training')
    testing_folder_path = ops.join(to_dir, 'testing')

    os.makedirs(traing_folder_path, exist_ok=True)
    os.makedirs(testing_folder_path, exist_ok=True)

    for json_label_path in glob.glob('{:s}/train*.json'.format(src_dir)):
    ...
    gen_train_sample(to_dir, gt_binary_dir, gt_instance_dir, gt_image_dir)

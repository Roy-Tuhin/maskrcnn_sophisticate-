...
exports._convertViaToTuSimple.prototype.init = function () {
  ...
  for (var one of opts.inFile) {
    ...
    this.convertOneFile(opts,one);
  }
}


exports._convertViaToTuSimple.prototype.getLaneH = function (opts,anno,refSeg) {
  var lanesH = {};
  var errors = {};
  var ri = 0;
  for (var one of anno.regions) { 
    ...
    if (one.shape_attributes.name != 'polyline') {
      this.printBoth("Error: Only polylines are allowed ",{image:anno.filename,label:idx,idx:ri,one});
      continue;
    }
    var sax = one.shape_attributes.all_points_x;
    var say = one.shape_attributes.all_points_y;
    if (sax.length != say.length) {
      this.printBoth("Error: x y dont match ",{image:anno.filename,label:idx,idx:ri,one});
    }
    var setA = [];
    var last = null;
    ...
    for (var oneRef of refSeg) {
      ...
      for (var oneSeg of setA) {
        ...
        if (t.length > 1) { 
          this.printLog("Error multiple seg cut ",{image:anno.filename,label:idx,idx:ri,oneSeg,oneRef}); 
          var err = "Error multiple seg cut "+anno.filename+" idx "+ri;
          errors[err] = 1;
          continue; 
        }
        isect.push(t[0]);
      ...
      if (isect.length == 2 && 
          Math.round(isect[0].x) == Math.round(isect[1].x)) {
        isect.pop();
      }
      if (isect.length > 1) { 
        this.printLog("Error multiple lane cut ",{image:anno.filename,label:idx,idx:ri,setA,oneRef,isect}); 
        var err = "Error multiple lane cut "+anno.filename+" idx "+ri;
        errors[err] = 1;
        lp.push(-2);
        continue; 
      }
      lp.push(Math.round(isect[0].x));
    }
    //this.printLog({lp});
    lanesH[idx] = lp;
  }
...
exports._convertViaToTuSimple.prototype.checkOutFile = function (opts,outFile) {
  ...
  var err = [];
  for (var one of anno) {
    if (!one.lanes || !one.h_samples || !one.raw_file) {
      err.push("Improper fields in "+one.raw_file);
      continue;
    }
    for (var i in one.lanes) {
      if (one.lanes[i].length != one.h_samples.length) {
        err.push("Lane idx "+i+" length does not match with h_samples length in "+one.raw_file);
      }
    }
  }
  if (err.length) {
...
exports._convertViaToTuSimple.prototype.doViaFormatting = function (opts,anno,refSeg,blankLane) {
  var count = 0;
  var retLanes = [];
  for (var key in anno) {
    if (!anno[key].regions) { continue; }
    count++;
    var lanesH = this.getLaneH(opts,anno[key],refSeg);
    this.printLog({lanesH});

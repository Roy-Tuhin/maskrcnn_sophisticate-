...
def numpy_to_json(json_input):
  """As Numpy Array Is Not Json serializable, it converts the numpy array to serialiazable json string
  It uses the custom class: NumpyEncoder
...
def createUUID(prefix='uid'):
  ...
  Some prefix used when generating UUIDs are:
  ant - annotation
  img - image
...
def dict_keys_to_lowercase_with_filter(d, fltr=None):
  """Converts all the dictionary keys to lowercase in recursive function call expect for the specific fltr key

  TODO: fltr as an list
  """
...
def dict_keys_to_lowercase(d):
  """Converts all the dictionary keys to lowercase in recursive function call
  """
